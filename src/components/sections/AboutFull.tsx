    ...
    >
      <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-start gap-14 lg:grid-cols-2 lg:gap-20">
        ...
        <div>
          ...
          <h2 className="font-display text-[clamp(32px,5vw,56px)] font-bold leading-[1.1] text-ink">
            Light is my
            <br />
            <em className="font-display italic text-gold">language.</em>
          </h2>
          <p className="mb-5 mt-6 text-[15px] leading-[1.8] text-ink/70">
          ...
          <p className="mb-9 text-[15px] leading-[1.8] text-ink/70">
          ...
          <button className="btn-outline mt-8 border-ink/15 text-ink hover:border-gold hover:text-gold">
        ...
        <div>
          {/* Skills */}
          <div className="mb-12">
            <p className="mb-6 text-xs uppercase tracking-[0.14em] text-ink/40">
              Core Skills
            </p>
            {SKILLS.map((skill) => (
              <div key={skill.name} className="mb-5">
                <div className="mb-2 flex justify-between">
                  <span className="text-[13px] text-ink/70">{skill.name}</span>
                  <span className="text-[13px] text-gold">{skill.pct}%</span>
                </div>
                <div className="h-0.5 rounded-sm bg-cream-border">
                  <div
          ...
          {/* Timeline */}
          <p className="mb-6 text-xs uppercase tracking-[0.14em] text-ink/40">Journey</p>
          <div className="flex flex-col gap-5">
            {TIMELINE.map((entry) => (
              <div key={entry.year} className="timeline-item on-light">
                <span className="text-[11px] font-semibold tracking-[0.08em] text-gold">
                  {entry.year}
                </span>
                <p className="mt-1 text-sm leading-[1.6] text-ink/70">{entry.event}</p>
              </div>
            ))}
          </div>
        </div>
